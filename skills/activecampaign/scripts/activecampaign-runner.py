...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'/tmp/activecampaign-' + datetime.now().strftime('%Y%m%d') + '.log')
    ]
)
logger = logging.getLogger(__name__)

# 核心功能
FEATURES = ['contacts', 'deals', 'tags', 'automations']
...
def main():
    parser = argparse.ArgumentParser(
        description='ActiveCampaign CRM集成',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s run --feature contacts
  %(prog)s status
  %(prog)s report
        """
    )
    ...
    if args.command == 'run':
        ...
        print("ActiveCampaign CRM集成系统状态:")

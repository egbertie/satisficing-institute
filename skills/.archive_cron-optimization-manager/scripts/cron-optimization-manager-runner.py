...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'/tmp/cron-optimization-manager-' + datetime.now().strftime('%Y%m%d') + '.log')
    ]
)
logger = logging.getLogger(__name__)

# 核心功能
FEATURES = ['merge', 'analyze', 'optimize']
...
def main():
    parser = argparse.ArgumentParser(
        description='Cron优化管理器',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s run --feature merge
  %(prog)s status
  %(prog)s report
        """
    )
